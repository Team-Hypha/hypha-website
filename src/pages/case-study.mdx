import hyphaArchitecture from '../images/hypha-architecture.jpg'
import storylineLogs from '../images/storyline-logs.jpg'
...
Amy is a senior engineer at Labra Cadabra. Here she is working on her local computer, adding a feature to their application. She has written code that takes an input and she expects a certain output but this isn’t what is happening.When she tests the new code, she finds that a certain input is producing a different output than she expects.Looking only at the output, it’s not obvious what’s wrong.
...
![](https://lh5.googleusercontent.com/haxMtlfN6GixWGlDuSm-WnhqlTkZGJX99cNnKSp4fRyF7jQnciA4OTmx7JRQ0OSat_0-Az4FeKlZlw0YV4LVZ5yCdOHjeJChuaXP5yhnHCPkmaB595PV0lpkF-qpJM2VWIPlJkjB)

She can use logging to understand why the code is behaving this way. She writes a few lines of code to output the extra information she needs. In other words,: she instruments her code to emit logs. She can use these extra details to pinpoint and fix the issue. And with these extra details she can pinpoint the issue and fix it.
...
In development, logs are usually output to the console without being saved anywhere. current process - meaning they show up on the console and are not saved anywhere. This is fine since the developer is actively watching the logs. But in production, the volume of logs becomes much higher, and it becomes impractical to try and actively monitor them. no one is actively watching the logs - and if they were, the volume of logs would likely be overwhelming. This means in production, logs need to be stored, - often to a file on the server.
...
_make these benefits a little more concis_
...
Amy and her team determin\* that distributed tracing is a great solution here because traces show the high-level overview of the execution path of a request.
...
How will distributed tracing help Amy and her colleagues debug their distributed system? The intuitive visual summary of a request moving through the system, gives immediate context for the overall request. In addition, the visual representation of the timin* of each span often elucidate* problems with a request in an obvious way.
...
- **There are reports of some errors in the analysis service and let’s say you go to investigate.**
- **A typical trace looks like the one on top, but then you discover another**
- **Something is strange. The lab test service keeps getting called repeatedly and these spans are very short compared to the others.**
- **Once again, you now have the context needed to dive in deeper and find the root cause of the issue.**
...
If the\* have instrumented their code to include user or request data in both their traces and logs, she may also be able to use these to correlate.
...
There are a number of ad hoc ways of correlating between logs and traces. But what Amy and her team would like to have is a way to seamlessly connect their log data and their trace data. They need a single context that gets attached to every log and trace as it is created and will travel with a request as it moves through their system. This will make it possible to go from a log to the associated trace and then from that trace back to all the logs that were generated in every service touched by that request. And they’ll need a system to create the telemetry, attach the context, and then handle and use all this data. Next we will explore what is necessary to build a telemetry pipeline that will collect, store, and make this data useful.
...
One of the major pieces they’re going to need is a telemetry pipeline that can handle their telemetry data.

In order to achieve their desired level of observability, Amy and her team will need to provision or build a telemetry pipeline.Let’s get an overview of the major stages that compose a telemetry pipeline so that we have a better idea of what’s involved before moving forward. We’re going to use the model outlined by Jamie Riedesel in her book*Software Telemetry*, which describes three major phases: Emitting, shipping, and presentation.

### Emit

The emitting stage iswhere we instrument the code to generate and emitadd the code that’s responsible for generating the telemetry data that will flow through the rest of the systemwe’re going to use. As mentioned earlier, this process is also referred to as instrumentation.

For example, to generate traces, one approach is to use an SDK from a framework like Open Telemetry. When a new request hits the systemservice, this code is responsible for generating the data that’s needed to start tracing the journey of that request through the system.
